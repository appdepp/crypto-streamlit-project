...
def inject_css() -> None:
    ...
    st.markdown(
        ...
        .tf-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.7rem;margin:.75rem 0 1rem}.tf-card{border:1px solid var(--line);background:rgba(16,23,34,.82);border-radius:18px;padding:.85rem;box-shadow:0 12px 30px rgba(0,0,0,.18)}.tf-head{display:flex;justify-content:space-between;align-items:center;font-weight:950}.tf-head span{color:var(--muted);font-size:.86rem}.tf-head strong{font-size:1.5rem;letter-spacing:-.05em}.tf-signal{font-weight:950;margin:.2rem 0 .25rem}.tf-line{color:var(--muted);font-size:.82rem;line-height:1.45}.tf-track{height:8px;border-radius:999px;background:rgba(255,255,255,.08);overflow:hidden;margin-top:.65rem}.tf-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,var(--red),var(--yellow),var(--green))}.tf-summary{font-weight:950;font-size:1.05rem;text-align:right}.tf-list{margin-top:.75rem}.compact-asset .asset-price{font-size:2.1rem}.stSidebar [data-testid="stSidebarNav"]{display:none!important}
